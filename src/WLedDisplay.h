...
class WLedDisplay : public WDevice {
 public:
  WLedDisplay(WNetwork* network)
      : WDevice(network, DEVICE_ID, network->getIdx(),
                DEVICE_TYPE_ON_OFF_SWITCH) {
    this->mx = new WMax7219(DIN_PIN, CS_PIN, CLK_PIN, NUM_SEGMENTS);
    this->addPin(mx);
    // On
    this->onProperty = WProperty::createOnOffProperty("on", "Power");
    this->onProperty->setBoolean(false);
    this->onProperty->setOnChange(std::bind(&WLedDisplay::onPropertyChanged,
                                            this, std::placeholders::_1));
    this->addProperty(onProperty);
    // Streamtext
    this->title = WProperty::createStringProperty("title", "Title");
    this->title->setOnChange(
        std::bind(&WLedDisplay::titleChanged, this, std::placeholders::_1));
